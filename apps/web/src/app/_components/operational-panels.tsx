...
function ResultMessage({
  ...
  return (
    <div
      role="status"
      className={`rounded-control border px-4 py-3 text-sm ${error ? 'border-critical/20 bg-critical-subtle text-critical-text' : 'border-positive/20 bg-positive-subtle text-positive-text'}`}
    >
      {message}
    </div>
  );
...
export function ParticipationPanel({
  ...
  const [score, setScore] = useState(
    participation.operatorScore === null ? '' : String(participation.operatorScore),
  );
  ...
  return (
    <SidePanel open={open} onOpenChange={setOpen}>
      ...
      <SidePanelContent
        title={`Participation · ${startupName}`}
        description="Manage this startup's cycle request, operational score, and published explanation."
        width="lg"
      >
        <SidePanelBody className="space-y-7">
          <section className="grid gap-4 sm:grid-cols-2">
            ...
            <TextField
              ...
              hint="Internal 0–100 score used by fixture prioritization."
            />
            <TextField
          ...
          <TextField
            ...
            hint="Comma separated, for example software, data, product."
          />
          <div className="grid gap-5 border-t border-hairline pt-6 sm:grid-cols-2">
            <TextAreaField
              ...
              hint="Visible to QSTP only. Never included in the startup decision."
              rows={7}
            />
            <TextAreaField
              label="Startup-facing justification"
              value={justification}
              onChange={(event) => setJustification(event.target.value)}
              hint="Published with the allocation tier. Do not include comparative scoring."
              rows={7}
            />
          </div>
        ...
        <SidePanelFooter>
          <p className="text-xs text-ink-3">Every change appends an activity event.</p>
  ...
  return (
    <SidePanel open={open} onOpenChange={setOpen}>
      <Button variant="primary" onClick={() => setOpen(true)}>
        Review stage gate <ArrowRight />
      </Button>
      <SidePanelContent
        title={`Advance to ${next.replaceAll('_', ' ')}`}
        description={`Review the gate from ${stage.replaceAll('_', ' ')} before changing the cycle lifecycle.`}
        width="md"
      >
        <SidePanelBody className="space-y-6">
          <div className="space-y-2">
            {checks.map((check) => (
              <div
                key={check.key}
                className="flex items-start gap-3 rounded-control border border-hairline px-4 py-3"
              >
                <span
                  className={`mt-0.5 flex size-5 items-center justify-center rounded-full ${check.passed ? 'bg-positive-subtle text-positive-text' : check.severity === 'blocking' ? 'bg-critical-subtle text-critical-text' : 'bg-warning-subtle text-warning-text'}`}
                >
                  {check.passed ? (
          ...
          {warnings.length > 0 && blockers.length === 0 ? (
            <TextAreaField
              label="Warning override reason"
              required
              value={reason}
              onChange={(event) => setReason(event.target.value)}
              hint="Explain why advancing is safe. This reason is written to the immutable activity timeline."
              rows={5}
            />
          ) : null}
          {blockers.length > 0 ? (
            <div className="rounded-control border border-critical/20 bg-critical-subtle px-4 py-3 text-sm text-critical-text">
              <p className="font-semibold">This gate is blocked.</p>
              <p className="mt-1 text-xs">
                Resolve {blockers.length} blocking check{blockers.length === 1 ? '' : 's'} before
                advancing.
              </p>
            </div>
          ) : null}
        ...
        <SidePanelFooter>
          <div className="flex items-center gap-2 text-xs text-ink-3">
            ...
            Stage changes are permanent and audited.
          </div>
          <div className="flex gap-2">
            <SidePanelClose asChild>
              <Button variant="ghost">Cancel</Button>
            ...
            <Button
              variant="primary"
              disabled={pending || blockers.length > 0 || (warnings.length > 0 && !reason.trim())}
              onClick={() =>
                startTransition(async () => {
                  const result = await advanceCycleStageAction({
                    cycleId,
                    to: next,
                    warningOverrideReason: warnings.length > 0 ? reason : null,
                  });
            ...
            >
              {pending ? 'Advancing…' : `Advance to ${next.replaceAll('_', ' ')}`}
            </Button>
          </div>
        </SidePanelFooter>
      </SidePanelContent>
